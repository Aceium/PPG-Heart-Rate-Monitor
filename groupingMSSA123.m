function [ restored ] = groupingMSSA123(ppgSample1, ppgSample2) % for learning , pcNumIn )
...
pcNumIdeal = 0;
eigSum = sum((log10(diag(eigV))));
eigNum = 0;
currentEigV = 1;

while (eigNum/eigSum <= 0.70)
...
for i = 1:pairNumIdeal
    if (pairsToSum(i)) == 1
        groupedPCs = groupedPCs + U(:,i)*S(i,i)*V(:,i)' + U(:,i+1)*S(i+1,i+1)*V(:,i+1)';
